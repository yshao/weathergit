...
class SmapUtils(object):
    ...
    def get_smap_time(self,target):
        self._init_smap_tool()

        if target == 'weather.ini':
            l= test_liveness(self.cfg['smap_source_mon'], self.opts)
        elif target == 'trendnet.ini':
            print self.cfg['smap_ipcam_mon']
            l= test_liveness(self.cfg['smap_ipcam_mon'], self.opts)
    ...
    def get_smap_time_dict(self,target):
        ""
        lines=self.get_smap_time(target)
        d={}
        for l in lines:
            sl=re.split(r' *', l)
...
if __name__ == '__main__':
    ...
    su=SmapUtils()
    lines=su.get_smap_time('weather.ini')
    d={}
    for l in lines:
        sl=re.split(r' *', l)
